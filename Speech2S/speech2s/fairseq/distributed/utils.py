...
def infer_init_method(cfg: DistributedTrainingConfig, force_distributed=False):
    ...
    if "AML_RUN_ID" in os.environ:
        if "OMPI_COMM_WORLD_SIZE" in os.environ:
            _infer_aml_distributed_launch_init(cfg)
        else:
            logger.info("No OMPI detected. Fall back to single node training")
...
def _infer_aml_distributed_launch_init(cfg: DistributedTrainingConfig):
    master_ip = os.environ.get("MASTER_IP")
    master_port = os.environ.get("MASTER_PORT") or "6000"
    cfg.distributed_init_method = f"tcp://{master_ip}:{master_port}"
    cfg.distributed_world_size = int(os.environ["OMPI_COMM_WORLD_SIZE"] or 1)
    cfg.distributed_rank = int(os.environ["OMPI_COMM_WORLD_RANK"] or 0)
    cfg.device_id = int(os.environ["OMPI_COMM_WORLD_LOCAL_RANK"] or 0)
    # processes are created by openmpi
    cfg.distributed_no_spawn = True
    logger.info(cfg.distributed_init_method)
