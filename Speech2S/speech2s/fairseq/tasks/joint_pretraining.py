...
@register_task("joint_pretraining", dataclass=STbertPretrainingConfig)
class JSTPretrainingTask(FairseqTask):
    ...
    def load_dataset(self, split: str, epoch=1, **kwargs) -> None:
        ...
        """
        ### , create a speech dataset using STSpeechDataset (modified from HubertDataset)
        ...
        speech_dataset = HubertDataset(
            f"{self.cfg.data}/{split}.tsv",
        ...
        has_unpaired_text = os.path.exists(f"{self.cfg.text_cfg.text_data}/{split}.idx")
        logger.info(f"split {split} has unpaired text? {has_unpaired_text}")
        if not has_unpaired_text:
            assert speech_dataset is not None
            self.datasets[split] = speech_dataset
            return

        ### 2nd, create a text dataset using StripTokenDataset (see examples/bart and examples/wav2vec/unuspervised for details)
        ...
        speech_dataset, [mono_dataset], [], [split], [split], epoch=epoch,
        )
