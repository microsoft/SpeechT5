...
class EpochBatchIterator(EpochBatchIterating):
    ...
    def _get_iterator_for_epoch(
        ...
        def _inner_bucket_shuffle(batches, bucket_size=10, thr=0):
            ...
            i = 0
            logger.info(f"_inner_bucket_shuffle skips the head/tail {int(thr * num_buckets)*bucket_size} batches")
            while i < num_batch:
                if i < bucket_size * int(thr * num_buckets) or i >= bucket_size * int((1-thr) * num_buckets):
                    new_batches.append(batches[i])
                    i += 1
                else:
                    bucket = np.concatenate(batches[i: i+bucket_size])
                    np.random.shuffle(bucket)
                    new_batches += np.array_split(bucket, bucket_size)
                    i += bucket_size
        ...
        def shuffle_batches(batches, seed):
            with data_utils.numpy_seed(seed):
                ...
                if self.grouped_shuffling:
                    ### to support better shuffling, we first collect data into buckets and shuffle them inner buckets
                    batches = _inner_bucket_shuffle(batches, bucket_size=10*self.num_shards, thr=0.01)
                    grouped_batches = [
