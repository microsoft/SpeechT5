...
@dataclass
class CodebertCriterionConfig(FairseqDataclass):
    ...
    no_ctc_blank: bool = field(
        default=False,
        metadata={"help": "mask out the blank of ctc"},
    )
    compute_target: bool = field(
        default=False,
        metadata={"help": "mask out the blank of ctc"},
    )


@register_criterion("codebert_criterion", dataclass=CodebertCriterionConfig)
class CodeBertCriterion(FairseqCriterion):
    def __init__(
        self, 
        task, 
        pred_masked_weight, 
        ...
        self.padding_idx = task.src_dict.pad()
        self.eos_idx = task.src_dict.eos()
        self.blank_idx = task.src_dict.bos()
    ...
    def forward(self, model, sample, reduce=True, log_pred=False):
        ...
        sample["net_input"]['padding_mask'] = sample["net_input"]['src_tokens'] == self.padding_idx
        net_output = model(**sample["net_input"])

        if self.compute_target:
            sample["net_input"] = {"src_tokens": sample["target"], "padding_mask": sample["target"] == self.padding_idx}
            net_output_2 = model(**sample["net_input"])
            net_output["logit_m_list"] += net_output_2["logit_m_list"]
            net_output["logit_u_list"] += net_output_2["logit_u_list"]
            net_output["padding_mask"] = [net_output["padding_mask"], net_output_2["padding_mask"]]
        ...
        if "decoder_target" in sample:
            ...
            dec_loss, dec_sample_size = self.compute_ctc_loss(model, net_output, sample, reduction=reduction)
            ...
            #     n_correct, total = self.compute_accuracy(model, net_output["encoder_out_ctc"], sample)
    ...
    def compute_ctc_loss(self, model, net_output, sample, reduction):
        ...
        sample_size = sample["dec_ntokens"]
