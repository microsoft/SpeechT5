...
@register_criterion("hst", dataclass=HSTCriterionConfig)
class HSTbertCriterion(FairseqCriterion):
    def __init__(
        self, 
        task, 
        pred_masked_weight, 
    ...
    def forward(self, model, sample, reduce=True, log_pred=False):
        ...
        if text_sample is not None:
            ...
            text_sample["net_input"]["target_list"] = [
                text_sample["net_input"]["src_tokens"].clone(),
                text_sample["ref_tokens"],
            ]
            text_net_output = model(**text_sample["net_input"])
            ### 2.1 U2T loss (masked mlm)
            ...
            loss += self.text_weight * loss_u2t * sample_size / sample_size_u2t
            ...
            if self.text_ctc_weight > 0:
                ...
                text_ctc_loss = 0
            loss = loss + (self.text_weight * self.text_ctc_weight * text_ctc_loss) *  sample_size / text_sample_size
