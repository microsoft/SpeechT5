...
class HubertCriterionConfig(FairseqDataclass):
    ...
    dec_loss_type: str = field(
        default="ctc",
        metadata={"help": "choice from (ctc, ce)"},
    )
    ...
    mum_weight: float = field(
        default=1.0,
        metadata={"help": "masked unit modeling weight"},
    )
...
@register_criterion("sc2t", dataclass=HubertCriterionConfig)
class SC2TCriterion(FairseqCriterion):
    def __init__(
        self, 
        task, 
        pred_masked_weight, 
    ...
    def forward(self, model, sample, reduce=True, log_pred=False):
        ...
        if "net_input" in sample:
            sup_sample = code_sample = text_sample = None
        else:
            speech_sample = sample.get("speech", None)
            sup_sample = sample.get("speech_sup", None)
            code_sample = sample.get("text_mono", None)
            text_sample = sample.get("text_paired", None)
            assert speech_sample is None or sup_sample is None
            assert code_sample is not None or text_sample is not None
        ...
        if sample is not None:
            ...
            if "decoder_target" in sample:
                ### decoder here means code-encoder+CTC
                if self.dec_loss_type == "ctc":
                    dec_loss = self.compute_ctc_loss(model, net_output, sample["decoder_target"], reduction=reduction)
                    dec_sample_size = sample["dec_ntokens"]
                else:
                    lprobs = F.log_softmax(net_output['encoder_out_ctc'][0].transpose(0, 1).float(), dim=-1)
                    target = net_output['target']
                    mask = target.ne(self.padding_idx)
                    dec_loss, dec_nll_loss = label_smoothed_nll_loss(
                        lprobs,
                        target,
                        epsilon=0,
                    ...
                    dec_sample_size = utils.item(mask.sum())
                    if self.report_accuracy:
                        n_correct = lprobs.argmax(-1).masked_select(mask).eq(target.masked_select(mask)).sum()
                        logging_output["dec_n_correct"] = utils.item(n_correct.data)
                        logging_output["total"] = dec_sample_size

                loss = loss + (self.dec_weight * dec_loss *  sample_size / dec_sample_size)
                logging_output["dec_loss"] = utils.item(dec_loss.data)
                logging_output["dec_sample_size"] = dec_sample_size
        
        else:
            sup_sample["modality"] = "speech_sup"
            sup_sample["target"] = sup_sample["decoder_target"]
            sup_net_output = model(**sup_sample["net_input"], supervised_batch=True)
            if self.text_ctc_weight < 1:
                sup_dec_loss, sup_dec_nll_loss = self.compute_ce_loss(model, sup_net_output["decoder_out"], sup_sample, reduce=reduce)
            else:
                sup_dec_loss, sup_dec_nll_loss = 0, 0
            if self.text_ctc_weight > 0:
                sup_ctc_loss = self.compute_ctc_loss(model, sup_net_output, sup_sample["target"], reduction=reduction)
                loss = sup_dec_loss + self.text_ctc_weight * sup_ctc_loss
                logging_output["sup_ctc_loss"] = utils.item(sup_ctc_loss)
            else:
                loss = sup_dec_loss
            sup_sample_size = sup_sample["ntokens"]
            logging_output["sup_dec_loss"] = utils.item(sup_dec_loss)
            logging_output["sup_dec_nll_loss"] = utils.item(sup_dec_nll_loss)
            logging_output["sup_sample_size"] = sup_sample_size

            if self.report_accuracy and self.text_ctc_weight < 1:
                n_correct, total = self.compute_accuracy(model, sup_net_output["decoder_out"], sup_sample)
                logging_output["sup_dec_n_correct"] = utils.item(n_correct.data)
                logging_output["sup_total"] = utils.item(total.data)
            sample = sup_sample
            sample_size = sup_sample_size

        
        ### prepare batch data for MUM loss
        if code_sample is not None:
            src_tokens = code_sample["net_input"]["src_tokens"]
            target = code_sample.get("target", src_tokens.clone())
            mum_results = model.forward_mum(src_tokens, target)
            ## the losses are provided token-averaged
            for name in mum_results:
                if name.startswith("loss_"):
                    p = mum_results[name] * sample_size
                    loss = loss + self.mum_weight * p
                    logging_output[name] = utils.item(p)
        ...
        if text_sample is not None:
            ...
            if self.text_ctc_weight < 1:
                ...
                text_dec_loss, text_dec_nll_loss = 0, 0
            if self.text_ctc_weight > 0:
            ...
            loss = loss + (self.text_weight * (text_dec_loss + self.text_ctc_weight * text_ctc_loss) *  sample_size / text_sample_size)

            ### 2.2 MUM loss
            if "loss_code_mlm" in text_net_output:
                loss_code_mlm = text_net_output["loss_code_mlm"] * sample_size 
                loss = loss + self.mum_weight * loss_code_mlm
                logging_output["loss_code_mlm"] = utils.item(loss_code_mlm)
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        if "sup_dec_loss" in logging_outputs[0]:
            sup_ctc_loss_sum = sum(log.get("sup_ctc_loss", 0) for log in logging_outputs)
            sup_dec_loss_sum = sum(log.get("sup_dec_loss", 0) for log in logging_outputs)
            sup_dec_sample_size = sum(log.get("sup_sample_size", 0) for log in logging_outputs)
            metrics.log_scalar(
                "sup_dec_loss", sup_dec_loss_sum / sup_dec_sample_size / math.log(2), sup_dec_sample_size, round=3
            )
            metrics.log_scalar(
                "sup_ctc_loss", sup_ctc_loss_sum / sup_dec_sample_size / math.log(2), sup_dec_sample_size, round=3
            )
            sup_total = utils.item(sum(log.get("sup_total", 0) for log in logging_outputs))
            if sup_total > 0:
                metrics.log_scalar("sup_total", sup_total)
                sup_n_correct = utils.item(
                    sum(log.get("sup_dec_n_correct", 0) for log in logging_outputs)
                )
                metrics.log_scalar("sup_dec_n_correct", sup_n_correct)
                metrics.log_derived(
                    "sup_dec_accuracy",
                    lambda meters: round(
                        meters["sup_dec_n_correct"].sum * 100.0 / meters["sup_total"].sum, 3
                    )
                    if meters["sup_total"].sum > 0
                    else float("nan"),
                )
