...
from fairseq.modules import GradMultiply, LayerNorm, PositionalEmbedding, TransposeLast
...
class HubertConfig(FairseqDataclass):
    ...
    complementary_mask: bool = field(
        default=False,
        metadata={"help": "use complementary mask"},
    )
    summary_method: str = field(
        default="conv",
        metadata={"help": "method to compute summary vector"},
    )
    summary_conv_kernel: int = field(
        default=64,
        metadata={"help": "complementary_conv kernel size"},
    )
    summary_dim: int = field(
        default=8,
        metadata={"help": "complementary_conv channel"},
    )
...
@register_model("cmbert", dataclass=HubertConfig)
class CMbertModel(BaseFairseqModel):
    def __init__(
        self,
        cfg: HubertConfig,
        ...
        self.summary_conv_kernel = cfg.summary_conv_kernel
        self.summary_dim = cfg.summary_dim
        ...
        if self.split_attention:
            assert not cfg.add_code_encoder, f"In case using shared attention, add_code_encoder should be set to False"
        ...
        if self.complementary_mask:
            if self.summary_method == "conv":
                conv = nn.Conv1d(cfg.encoder_embed_dim, cfg.summary_dim, cfg.summary_conv_kernel,
                                bias=False, padding=cfg.summary_conv_kernel // 2)
                nn.init.kaiming_normal_(conv.weight)
                self.complementary_extractor = nn.Sequential(
                    TransposeLast(),     # (B, T, D) --> (B, D, T)
                    conv,
                    nn.Dropout(p=0.1),
                    TransposeLast(),     # (B, D, T) --> (B, T, D)
                    LayerNorm(cfg.summary_dim),
                )
            elif self.summary_method == "pool":
                self.complementary_extractor = nn.Sequential(
                    TransposeLast(),     # (B, T, D) --> (B, D, T)
                    nn.AvgPool1d(cfg.summary_conv_kernel, stride=1, padding=cfg.summary_conv_kernel // 2),
                    TransposeLast(),     # (B, D, T) --> (B, T, D)
                    nn.Linear(cfg.encoder_embed_dim, cfg.summary_dim, bias=False),
                    LayerNorm(cfg.summary_dim),
                )
    ...
    def build_model(cls, cfg: HubertConfig, task: HubertPretrainingTask):
        ...
        model = CMbertModel(cfg, task.cfg, task.dictionaries, text_src_dictionary, text_tgt_dictionary)
        return model
    ...
    def forward_code_mlm(self, src_tokens):
        ...
        if self.code_use_hubert_logits:
            code_logit_m_list, _ = self.compute_hubert_logits(c_emb, target_list, self.code_encoder_proj, padding_mask, mask_indices)
    ...
    def compute_hubert_logits(self, x, target_list, proj, padding_mask, mask_indices):
    ...
    def forward(
        ...
        if source is not None:
            ...
            if self.complementary_mask:
                summary_vector = self.complementary_extractor(features.clone())
            ...
            if self.complementary_mask:
                x = torch.cat([x[:,:, :-self.summary_dim], summary_vector[:, :x.size(1), :]], dim=-1)
            ...
            logit_m_list, logit_u_list = self.compute_hubert_logits(x, target_list, self.final_proj, padding_mask, mask_indices)
            ...
            if self.add_code_encoder:
                ...
                if self.seek_code_embedding:
                    ...
                    if self.code_use_hubert_logits:
                        code_logit_m_list, _ = self.compute_hubert_logits(encoder_out['encoder_out'][0].transpose(0, 1), 
                                                                    target_list, self.code_encoder_proj, padding_mask, mask_indices)
                ...
                if self.compute_code_mlm:
                    ...
                    clm_loss = self.forward_code_mlm(reduced_target.long())
                    result['clm_loss'] = clm_loss
    ...
    def get_extra_losses(self, net_output):
        ...
        if "clm_loss" in net_output:
            extra_losses.append(net_output["clm_loss"])
            names.append("clm_loss")
