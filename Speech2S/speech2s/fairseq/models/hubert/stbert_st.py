...
class HubertAsrConfig(FairseqDataclass):
    ...
    finetune_decoder_params: Optional[str] = field(
        default=None,
        metadata={
            "help": "freeze decoder params"
        },
    )
...
@register_model("stbert_st", dataclass=HubertCtcConfig)
class STbertST(BaseFairseqModel):
    ...
    def build_model(cls, cfg: HubertCtcConfig, task: FairseqTask):
        ...
        # for k, p in w2v_encoder.named_parameters():
        #     # Freeze pretrained models by default
        #     kk = cfg.finetune_decoder_params.split('+')
        #     d1 = kk[0]
        #     d2 = kk[1]
        #     d3 = kk[2]
        #     if d1 not in k and d2 not in k and d3 not in k and "decoder" in k:
        #         logger.info(f"freeze_{k}")
...
#            for k, p in self.w2v_model.named_parameters():
            # Freeze pretrained models by default
