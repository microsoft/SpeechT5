...
@dataclass
class HSTbertConfig(FairseqDataclass):
...
@register_model("hstbert", dataclass=HSTbertConfig)
class HSTbertModel(BaseFairseqModel):
    """
    Different from Hubert, here the labels (in label_list) are present hierarchically, e.g.,
    Unit label for 6-layer output, Phone label for 12-layer output, and Char label for 12-layer + CTC output.
    """
    def __init__(
        self,
        cfg: HSTbertConfig,
        ...
        if self.add_code_encoder:
            ...
            assert len(code_dictionary) == self.num_classes[0], f"code_dictionary: {len(code_dictionary)}, self.num_classes[0]: {self.num_classes[0]}"
            ...
            ### change self.label_embs_list[0] to code_embeddings
            if final_dim == cfg.text_transformer.encoder.embed_dim:
                logger.info("Share label_embs with code_embed_tokens ...")
                nn.init.uniform_(self.code_embed_tokens.weight)
                self.label_embs_list[0] = self.code_embed_tokens.weight
        ...
        ### build text decoder, not available for now
    ...
    @classmethod
    def build_model(cls, cfg: HSTbertConfig, task: HubertPretrainingTask):
        """Build a new model instance."""
        text_src_dictionary = getattr(task, "text_src_dictionary", None)
        text_tgt_dictionary = getattr(task, "text_dictionary", None)
        model = HSTbertModel(cfg, task.cfg, task.dictionaries, text_src_dictionary, text_tgt_dictionary)
        return model
    ...
    def compute_hubert_logits(self, x, target, proj, label_embs, padding_mask, mask_indices):
        ...
        if not self.skip_masked:
            ...
            logit_m_list = [
                compute_pred(proj_x_m, target[masked_indices], label_embs)
            ]
        else:
            logit_m_list = [None]
        ...
        if not self.skip_nomask:
            ...
            logit_u_list = [
                compute_pred(proj_x_u, target[nomask_indices], label_embs)
            ]
        else:
            logit_u_list = [None]
        ...
        if source is not None:
            ...
            if self.moe_part != "none":
                ...
                if self.seek_code_embedding and not self.mix_with_hidden_unit:
                    _, code_embeddings, embedding_l2 = self.get_code_embeddings(code_embeddings, padding_mask, src_tokens, 
                        mask_indices=mask_indices,
                        pad_with_code=self.pad_with_code,
                        softmax=self.prob_code_embedding,
                        l2_embedding=self.l2_embedding,
    ...
    def forward_torchscript(self, net_input: Dict[str, Tensor]):
        ...
        x = res["x"] # B x T x D
        res["x"] = res["x"].transpose(0, 1) # (T, B, D)
    ...
    def extract_features(
        ...
        return feature, res["padding_mask"], None
